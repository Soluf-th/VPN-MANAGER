...
import { Shield, Power, Activity, Lock, RefreshCw, ShieldAlert, Bug, Timer, Award, Info, Search } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [cveSearchQuery, setCveSearchQuery] = useState<string>('');
  ...
  const handleSecurityAudit = async () => {
    ...
    setIsScanning(true);
    addLog('System Security Audit started...', 'info');
    addLog('Analyzing CILPEA instance against global CVE database...', 'info');

    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: "You are a professional Cybersecurity Auditor. Analyze a Linux server running OpenVPN 2.6 and identify 5 highly relevant security vulnerabilities or configuration gaps. Include real CVE IDs if applicable. Format as a strict JSON array of objects.",
        config: {
          ...
          responseSchema: {
            ...
            items: {
              ...
              properties: {
                ...
                status: { type: Type.STRING, enum: ["Open", "Mitigated", "Resolved", "In Review"] },
                category: { type: Type.STRING }
              },
              required: ["id", "cveId", "title", "severity", "description", "recommendation", "status", "category"]
            }
          }
        }
      });
      ...
      addLog(`Security audit finalized. ${data.length} findings require attention.`, 'warning');
    } catch (error) {
      addLog('Audit failed: Gemini API error.', 'error');
    } finally {
      setIsScanning(false);
    }
  };

  const handleCveLookup = async (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (isScanning || !cveSearchQuery.trim()) return;

    setIsScanning(true);
    addLog(`Initiating lookup for ${cveSearchQuery}...`, 'info');

    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `You are a cybersecurity expert. Provide a detailed report for ${cveSearchQuery}. If it's a real CVE, provide accurate details. If not found, provide a simulated relevant vulnerability for a VPN context. Return a JSON object matching the schema.`,
        config: {
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.OBJECT,
      ...
      const data = JSON.parse(response.text);
      setVulnerabilities(prev => [data, ...prev.slice(0, 4)]);
      addLog(`Report for ${data.cveId} generated successfully.`, 'success');
      setCveSearchQuery('');
    } catch (error) {
      addLog('Lookup failed: Gemini API error.', 'error');
    } finally {
      setIsScanning(false);
    }
  };
        ...
        <div className="lg:col-span-4 flex flex-col gap-6 overflow-y-auto pr-2 scrollbar-hide">
            ...
            <div className="flex justify-center mb-6 relative">
              <button
                ...
                className={`relative w-24 h-24 rounded-full flex items-center justify-center transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-slate-800 focus:ring-offset-2 focus:ring-offset-slate-950
                  ...
                  ${(status === ConnectionStatus.CONNECTING || status === ConnectionStatus.DISCONNECTING) ? 'opacity-50 cursor-not-allowed scale-95' : 'hover:scale-105 active:scale-95'}
                `}
              >
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-6 shadow-xl flex-1 flex flex-col min-h-[400px]">
            <div className="flex gap-4 border-b border-slate-800 mb-6">
              ...
              >
                <div className="flex items-center gap-1.5"><ShieldAlert className="w-3.5 h-3.5" /> Audit Findings</div>
            ...
            {activeTab === 'audit' ? (
              <>
                <form onSubmit={handleCveLookup} className="mb-4 relative group">
                  <input 
                    type="text" 
                    value={cveSearchQuery}
                    onChange={(e) => setCveSearchQuery(e.target.value)}
                    placeholder="Search CVE (e.g. CVE-2024-1234)..."
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg py-2 pl-8 pr-12 text-[10px] font-mono text-slate-300 focus:outline-none focus:border-emerald-500/50 transition-all placeholder:text-slate-700"
                  />
                  <Search className="absolute left-2.5 top-2.5 w-3 h-3 text-slate-600 group-focus-within:text-emerald-500 transition-colors" />
                  <button 
                    type="submit"
                    disabled={isScanning || !cveSearchQuery.trim()}
                    className="absolute right-1.5 top-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 p-1 rounded transition-colors disabled:opacity-30"
                  >
                ...
                <div className="flex justify-between items-center mb-4 px-1">
                    <p className="text-[9px] text-slate-500 uppercase font-mono tracking-widest flex items-center gap-1">
                      <Info className="w-3 h-3" /> System Vulnerability Monitor
                    </p>
                    <button 
                      onClick={handleSecurityAudit}
                      disabled={isScanning}
                      className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[9px] font-bold uppercase rounded border border-slate-700 transition-all flex items-center gap-1 disabled:opacity-50 active:scale-95"
                    >
                      <Bug className="w-3 h-3" />
                      {isScanning ? 'Scanning...' : 'Global Audit'}
                    </button>
                </div>
